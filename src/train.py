...
from src.config import NUM_CLASSES, ExpConfig
...
def run_training(cfg: ExpConfig, train_loader: DataLoader, val_loader: DataLoader, device: torch.device) -> nn.Module:
    ...
    for epoch in range(1, cfg.total_epochs + 1):
        ...
        train_loss = train_one_epoch(model, train_loader, criterion, optimizer, device, cfg)
        ...
        if epoch % 10 == 0 or epoch == cfg.total_epochs:
            logger.info(
                f"Epoch {epoch}/{cfg.total_epochs} | "
                f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | "
                f"QWK: {val_metrics['qwk']:.4f} | Macro F1: {val_metrics['macro_f1']:.4f}"
            )
